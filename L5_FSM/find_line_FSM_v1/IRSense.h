...
class IRSense {
   public:
      ...
      //This function needs an input - a globally defined int array of length 5. This will then be populated and
      //passed back.
      float* IRread(float ARRAY[5]) { 
           bool done = false;
           ...
           if (_bi[0] == 0) {
            ...
            ARRAY[1] = float(end_time[1]-start_time);
            ARRAY[2] = float(end_time[2]-start_time);
            ARRAY[3] = float(end_time[3]-start_time);
           }
           else {
            ARRAY[1] = (float(end_time[1]-start_time) - _bi[0])*_si[0];
            ARRAY[2] = (float(end_time[2]-start_time) - _bi[1])*_si[1];
            ARRAY[3] = (float(end_time[3]-start_time) - _bi[2])*_si[2];
           }
           //Populate other IR sensor values
           ARRAY[0] = float(end_time[0]-start_time);
           ARRAY[4] = float(end_time[4]-start_time);
      ...
      void _IRread() { 
        ...
        void beep_once(int pitch, int beep_length) {
          buzz.beep_once(pitch, beep_length);
        }
 };
