  ...
  public:
    ...
    void updateposition() {      
      ...
      if (travel_r == 0 || travel_l == 0) {
        ...
        x_local = (arc_radius)*sin(theta_local);
        Serial.println("one wheel");
                
      } else if (abs(travel_r) > abs(travel_l)) {
        arc_radius = (2*l*travel_l) / travel_r-travel_l;
        theta_local = -travel_r / (arc_radius+2*l);
        x_local = (arc_radius+l)*sin(theta_local); 
        Serial.println("r>l");
        
      } else if (abs(travel_r) < abs(travel_l)) {
        arc_radius = (2*l*travel_r / travel_l-travel_r);
        ...
        Serial.println("turn on spot");
               
      } else {
        theta_local = 0;
        x_local = (travel_r+travel_l)/2;    
        Serial.println("fwd/back");
      }
      
      theta_global = theta_global+theta_local;
      Serial.println(theta_global);
